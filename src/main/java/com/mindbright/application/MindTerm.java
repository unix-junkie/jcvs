...
public class MindTerm extends Applet implements Runnable, WindowListener {
  ...
  public void run() {
    try {
      ...
      if(weAreAnApplet &&
	 client.getPropertyHandler().getDefaultProperty("server") == null) {
	  client.getPropertyHandler().setDefaultProperty("server",
						 getCodeBase().getHost());
      }
      if(client.getPropertyHandler().getDefaultProperty("username") == null) {
	  try {
	      client.getPropertyHandler().setDefaultProperty("username",
	...
	if(!doSCP) {
	  ...
	  SSHSCPClient scp = new SSHSCPClient(srvHost, srvPort,
					      client.getPropertyHandler(),
					      (SSH.DEBUG ? client : null),
					      new File("."),
					      SSH.DEBUG);
	  if(SSH.DEBUG) {
